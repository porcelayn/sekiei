...
pub fn process_standard_images(markdown: &str, current_path: &Path) -> String {
    IMAGE_REGEX.replace_all(markdown, |caps: &regex::Captures| {
        let alt_text = &caps[1];
        let path = &caps[2];
...
pub fn process_alternative_images(markdown: &str, current_path: &Path) -> String {
    ALT_IMAGE_REGEX.replace_all(markdown, |caps: &regex::Captures| {
        let path = &caps[1];
...
pub fn process_links(markdown: &str) -> String {
    LINK_REGEX.replace_all(markdown, |caps: &regex::Captures| {
        let path = &caps[1];
...
pub fn process_wiki_parenthetical_links(markdown: &str) -> String {
    WIKI_LINK_REGEX.replace_all(markdown, |caps: &regex::Captures| {
    ...
    if let Some(file_map) = &*cache {
        if let Some(matches) = file_map.get(image_name) {
            match matches.len() {
                ...
                1 => {
                    let path = &matches[0];
                    format!("/static/{}", sanitize_filename(&path.strip_prefix("content").unwrap_or(path).to_string_lossy()))
                },
                _ => {
    ...
    if let Some(file_map) = &*cache {
        if let Some(matches) = file_map.get(link_name) {
            match matches.len() {
                ...
                _ => {
                    let match_path = matches.iter()
                        .find(|p| p.to_string_lossy().ends_with(".md"))
                        .unwrap_or(&matches[0]);
                    
                    let path = match_path.strip_prefix("content").unwrap_or(match_path).with_extension("");
...
pub fn resolve_path(path: &str, current_path: &Path) -> String {
    let current_dir = current_path.parent().unwrap_or(Path::new(""))
        .strip_prefix("content").unwrap_or(Path::new(""));
