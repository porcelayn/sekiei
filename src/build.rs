use std::{error::Error, fmt::format, fs, path::Path};
...
pub fn build() -> Result<(), Box<dyn Error>> {
    ...
    for entry in WalkDir::new("content").into_iter().filter_map(|e| e.ok()) {
        if entry.path().is_file() && entry.path().extension().and_then(|s| s.to_str()) == Some("md") {
            ...
            let mut output_path = dist.join(relative_path);
            // main page
            if relative_path.to_string_lossy().to_string().eq("index.md") {
                output_path = dist.join("index.html");
            } else {
                println!("Creating directory for {}", output_path.display());
                let output_dir: std::path::PathBuf = dist.join(relative_path.with_extension(""));
                create_directory_safely(&output_dir)?;
                println!("out dir: {}", output_dir.display());
                output_path = format!("{}{}", output_dir.display(), "\\index.html").into();
            }
            ...
            if let Err(e) = safely_write_file(&output_path, String::from_utf8(minified).unwrap().as_str()) {
